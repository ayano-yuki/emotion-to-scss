...
func scanTemplate(source string, start int) (string, []domain.Expression, int, error) {
	if start < 0 || start >= len(source) || source[start] != '`' {
		return "", nil, 0, fmt.Errorf("expected template literal")
	}
	...
	for i := start + 1; i < len(source); {
		...
		case '\\':
			...
			i += 2
		case '`':
			return out.String(), expressions, i + 1, nil
		case '$':
			if i+1 < len(source) && source[i+1] == '{' {
				...
				placeholder := fmt.Sprintf("__emotion_to_scss_dynamic_%d__", len(expressions)+1)
				expressions = append(expressions, domain.Expression{
	...
	return "", nil, 0, fmt.Errorf("unterminated template literal")
}
