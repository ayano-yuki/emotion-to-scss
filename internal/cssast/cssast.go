...
func (p *parser) parseNodes(stopOnBrace bool) ([]Node, error) {
	...
	for {
		...
		head, delimiter, err := p.readUntilTopLevel("{}")
		if err != nil {
			return nil, err
		}
...
func (p *parser) parseDeclarations() ([]Declaration, error) {
	...
	for {
		...
		statement, delimiter, err := p.readUntilTopLevel(";}")
		if err != nil {
			return nil, err
		}
...
func (p *parser) readUntilTopLevel(delimiters string) (string, byte, error) {
	start := p.pos
	var quote byte
	parenDepth := 0
	bracketDepth := 0
	for !p.eof() {
		...
		default:
			if parenDepth == 0 && bracketDepth == 0 && strings.ContainsRune(delimiters, rune(c)) {
				return p.input[start:p.pos], c, nil
			}
		}
		p.pos++
	}
	return p.input[start:p.pos], 0, nil
}
